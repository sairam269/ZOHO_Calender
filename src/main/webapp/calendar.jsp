<%@ page import="com.ZOHO.ZOHO_ShedularApp.service.Month,com.ZOHO.ZOHO_ShedularApp.model.UserModel,com.ZOHO.ZOHO_ShedularApp.web.UserNotifications,java.util.*,java.io.*,java.sql.*" errorPage="error.jsp" %>
...
<%
{
  ...
  un.fetchNotificationDates(Integer.parseInt(currentMonthString),Integer.parseInt(currentYearString));
  /*for (int[] u: days) {
	    for (int elem: u) {
  ...
  for( int i=0; i<aMonth.getNumberOfWeeks(); i++ )
  {
    ...
    for( int j=0; j<7; j++ )
    {
      ...
      else
      {
        ...
        {
        	if(un.checkDate(days[i][j])==1){
        		%><td style="color: cornflowerblue" class="today_cell"><%=days[i][j]%></td><%
        	}else{
        ...
        {
        	if(un.checkDate(days[i][j])==1){
        		%><td style="color: cornflowerblue" class="day_cell"><%=days[i][j]%></td><%
        	}else{
